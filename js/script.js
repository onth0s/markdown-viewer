...
let contentOffsetInRaw = (token) => {
  // Number of raw characters before inline content starts (structural prefix like '## ')
  if (!token.tokens || !token.tokens.length) return 0;
  let sumRaws = token.tokens.reduce((s, t) => s + (t.raw ? t.raw.length : 0), 0);
  let off = token.raw.length - 1 - sumRaws; // trailing \n
  return off < 0 ? 0 : off;
};
  ...
  for (let tok of tokens) {
    ...
    if (tokStart < localEnd && tokEnd > localStart) {
      ...
      if (tok.type === 'text' && (!tok.tokens || !tok.tokens.length)) {
        // Leaf text token — split at boundary if partially covered
        if (fullyCovered) {
          tok._sel = true;
        } else {
        ...
        // Codespan — split at content boundary, accounting for backtick delimiters
        ...
        // Container (strong, em, text with children, etc.) — highlight entirely
        tok._sel = true;
        // Only recurse into children on partial coverage (full coverage already highlights everything inside)
...
let markPartial = (token, localStart, localEnd) => {
  if (hasInlineChildren(token)) {
    ...
    // Fenced code block — line-level precision
    let lang = (token.lang || '').match(/^\S*/)?.[0].toLowerCase();
...
let renderWithSelection = (md, selStart, selEnd) => {
  ...
  for (let token of tokens) {
    ...
    if (tokenStart < selEnd && tokenEnd > selStart) {
      // Content end excludes trailing \n (user selection rarely includes it)
      let contentEnd = tokenEnd;
      if (rawLen > 0 && token.raw[rawLen - 1] === '\n') contentEnd--;
      if (tokenStart >= selStart && contentEnd <= selEnd) {
        // Use inline precision for blocks with inline children to avoid margin fill issues (headings, etc.)
        if (hasInlineChildren(token)) {
  ...
  let textWrapper = (fn) => {
    return function (token, ...args) {
      let html = fn.call(this, token, ...args);
      if (token && token._sel) {
        if (typeof token._sel === 'object') {
          let a = html.substring(0, token._sel.splitStart);
          let b = html.substring(token._sel.splitStart, token._sel.splitEnd);
          let c = html.substring(token._sel.splitEnd);
          return a + '<span class="preview-selection">' + b + '</span>' + c;
        }
  ...
  wrap('codespan', (fn) => {
    return function (token, ...args) {
      if (token && token._sel && typeof token._sel === 'object') {
        ...
        let e = Math.max(0, Math.min(t.length, token._sel.splitEnd));
        let parts = [];
        if (s > 0) parts.push('<code>' + escapeHtml(t.substring(0, s)) + '</code>');
        parts.push('<span class="preview-selection"><code>' + escapeHtml(t.substring(s, e)) + '</code></span>');
        if (e < t.length) parts.push('<code>' + escapeHtml(t.substring(e)) + '</code>');
        return parts.join('');
      }
      let html = fn.call(this, token, ...args);
      if (token && token._sel) {
...
let updatePreviewSelection = () => {
  ...
  lastSelEnd = end;
  if (start !== end) {
    convert(editor.value, start, end);
  } else if (output.querySelector('.preview-selection')) {
    convert(editor.value);
  }
};

editor.addEventListener('keyup', () => { updateCaretIndicator(); updatePreviewSelection(); });
editor.addEventListener('keydown', () => {
  queueMicrotask(updatePreviewSelection);
});
editor.addEventListener('mousedown', () => setTimeout(() => { updateCaretIndicator(); updatePreviewSelection(); }, 0));
editor.addEventListener('mouseup', () => setTimeout(() => { updateCaretIndicator(); updatePreviewSelection(); }, 0));
...
editor.addEventListener('select', () => { updateCaretIndicator(); updatePreviewSelection(); });
